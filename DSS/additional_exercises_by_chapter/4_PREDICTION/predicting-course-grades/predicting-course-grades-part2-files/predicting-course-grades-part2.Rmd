---
title: |
    | \vspace{-.8cm}Predicting Course Grades Using Midterm Scores  \newline Part II: Predicting Overall Scores \if1\solutions(with Solutions) \fi
output:
  pdf_document:
    keep_tex: false
    includes:
...
fontsize: 11pt
---
\vspace{-.5cm} Let's continue to analyze real, historical, student performance data from the class. The goal here is to model the relationship between midterm and overall scores so that we can later predict overall scores based on midterm scores. The dataset we will use is in the \textit{grades.csv} file. Table \ref{grades} shows the names and descriptions of the variables in this dataset, where the unit of observation is students.
...
\if1\solutions 
\newline\newline\underline{Answer}: *overall*, which is a non-binary variable
\newline
\fi
...
\newline\newline\underline{R code}:
        ...
        cor(grades$overall, grades$midterm) # computes correlation
        ```
...
\newline\newline\underline{Answer}: Yes, the relationship between the two variables is moderately to strongly linear since the correlation coefficient is much closer to 1 than to 0.
\newline
\fi
...
\newline\newline\underline{R code}:
        ...
        fit <- lm(grades$overall ~ grades$midterm) 
        ```
        ```{r, collapse=TRUE, eval=TRUE}
        fit # provides contents of object
        ```
(Recall: The function \fu{lm()} fits a linear model. It requires a function of the type \fu{Y $\sim$ X}, where Y identifies the Y variable (*overall*, in this case) and X identifies the X variable (*midterm*, in this case). To specify the dataframe where the variables are stored, we can use either the \fu{\$} operator (as in the code above) or the optional argument \fu{data}. If we wanted to use the latter, the code to fit the linear model would be \fu{lm(overall $\sim$ midterm, data = grades)}.)\newpage
\fi
...
\if1\solutions
\newline\newline\underline{Answer}: $\widehat{\textrm{overall}}$ = `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0)` + `r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)` *midterm*
\newline\newline (Note: The Y variable is *overall*, $\widehat{\alpha}$= `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0)`, $\widehat{\beta}$=`r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)`, and the X variable is *midterm*.)
\newline
\fi
...
\end{align*}
\newline\underline{Answer}: If I earn 80 points in the midterm, I would predict that I will earn `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*80` points overall in the class, on average. (Note: $\widehat{Y}$ is in the same unit of measurement as $\overline{Y}$; in this case, Y is non-binary and measured in points so $\overline{Y}$ and $\widehat{Y}$ are also measured in points. Note: We could have arrive at this same conclusion by looking at the scatter plot with the fitted line above. All we would need to do is (i) find 80 on the X-axis, (ii) go up to the line, and (iii) find the value on the Y-axis associated with that point on the line.)
\newline
\fi
    b. Computing $\widehat{Y}$ based on $X$: Now, suppose that you earn 90 points in the midterm. What would be your best guess of your predicted overall score in the class based on your performance in the midterm? Please show your calculations and then answer the question with a full sentence (including units of measurement). (5 points)
\if1\solutions
\newline\newline\underline{Calculations}: 
\begin{align*}
\widehat{\textrm{overall}} &= \widehat{\alpha} + \widehat{\beta} \, \textrm{midterm}\\
&= `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0)` + `r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)` \, \textrm{midterm}\\
&= `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0)` + `r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)` \times 90 \color{gray} \,\,\,\textrm{(if midterm=90)}\\
&= `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0)` + `r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*90`= `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*90`
\end{align*}
\newline\underline{Answer}: If I earn 90 points in the midterm, I would predict that I will earn `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*90` points overall in the class, on average. (Note: $\widehat{Y}$ is in the same unit of measurement as $\overline{Y}$; in this case, Y is non-binary and measured in points so $\overline{Y}$ and $\widehat{Y}$ are also measured in points.  Note: We could have arrive at this same conclusion by looking at the scatter plot with the fitted line above. All we would need to do is (i) find 90 on the X-axis, (ii) go up to the line, and (iii) find the value on the Y-axis associated with that point on the line.)
\newline
\fi
    c. Computing $\triangle \widehat{Y}$ based on $\triangle X$:  What is the predicted change in overall exam scores associated with an increase in midterm scores of 10 points? Please show your calculations and then answer the question with a full sentence (including units of measurement). (10 points)
\if1\solutions
\newline\newline\underline{Calculations}: 
\begin{align*}
\triangle \widehat{\textrm{overall}} &= \widehat{\beta} \, \triangle \textrm{midterm} \\
&= `r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)`  \times \triangle \textrm{midterm}\\
&= `r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)`  \times 10 \color{gray} \,\,\,\textrm{(if $\triangle$ midterm=10)}\\
&= `r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*10` 
\end{align*}
\newline\underline{Answer}:
An increase in midterm scores of 10 points is associated with a predicted increase in overall scores of `r round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*10` points, on average.
(Note: $\triangle\widehat{Y}$ is in the same unit of measurement as $\triangle\overline{Y}$; in this case, Y is non-binary and measured in points so $\triangle\overline{Y}$ and $\triangle\widehat{Y}$ are also measured in points.  Note: We could have arrive at this same conclusion by looking at the scatter plot with the fitted line above. All we would need to do is (i) pick two values on the X-axis that differ by 10, for example: 90 and 80, (ii) find the values on the Y-axis associated with each of those two points, in this case: `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*90` and `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*80` and (iii) compute the difference between those two Y values: `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*90` - `r round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*80` =  `r (round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*90) - (round(coef(lm(grades$overall ~ grades$midterm))[1], 0) + round(coef(lm(grades$overall ~ grades$midterm))[2], 2)*80)`.)
\newline
\fi
...
\newline\newline\underline{R code}:
    ```{r, collapse=TRUE, eval=TRUE}
    cor(grades$overall, grades$midterm)^2 # computes R^2
    ```
\underline{Answer}: The linear model using *midterm* as a predictor explains `r round(cor(grades$overall, grades$midterm)^2,2)*100`\% of the variation of overall scores. (Note: $R^2$ measures the proportion of the variation of the outcome variable explained by the model. In the simple linear model: $R^2$ = cor(X,Y)$^2$. Since $R^2$ is relatively close 1, it looks like it is a decent predictive model. As we can see in the scatter plot above, the prediction errors---the vertical distance between the dots and the line---are relatively small.)
\fi    
