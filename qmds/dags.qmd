...
\end{array}
$$
Repitamos el ejercicio de simulación que utilizamos en el ejemplo de los confusores, sólo que esta vez Covid es una variable dicotómica y, por lo tanto, debemos muestrarla de una distribución Bernoulli[^binomial]:

[^binomial]: Para más detalles sobre como simular una variiable con distribución Bernoulli (binomial con $n=1$) podemos ver el siguiente [link](https://library.virginia.edu/data/articles/simulating-a-logistic-regression-model).
...
```{r}
#| echo: false
#| fig-cap: "Infefctados y no infectados de COVID-19 en función de la edad."
#| fig-width: 6
#| fig-align: center
#| warning: false
#| message: false
...
Analicemos el ejemplo que plantean los autores en [@tonnies2022collider]. Lo que quieren estudiar es el efecto causal de la obesidad (**Ob**) end la mortalidad (**Mort**). Pero, identifican dos variables que tambièn podrìan estar afectando: 1- Si el individuo tiene diabetes (**Diab**) y 2- Si el individuo es fumador (**Smok**). La relación causal enntre las variables que plantean (medio de juguete) puede verse representada en el siguiente DAG[^dags-2]:

[^dags-2]: Recuerden que el diagrama causal no es un problema estadísticon sino más bien se plantea previo a cualquier consideración estadística, usando como insumo el conocimiento del dominio que tenemos.

```{r}
#| echo: true
#| fig-cap: "DAG que representa la relación causal planteada por los autores en [@tonnies2022collider]."
#| fig-width: 6
#| fig-align: center
...
[^dags-2]: Esto resulta especialmente útil cuando los DAGS se empiezan a complicar.
...
```{r}
#| echo: true
#| fig-cap: "Caminos causales abiertos luego de controlar por Diab."
#| fig-width: 8
#| fig-align: center
