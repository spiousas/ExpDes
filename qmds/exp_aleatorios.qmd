...
¿Pero no era que el efecto real era de $5$ segundos y que el $\hat\beta_T$ era un estimador insesgado del efecto porque se trata de un experimento aleatorio?[^exp_aleatorios-8] La respuesta es simple y complicada al mismo tiempo. Al simular los datos del experimento estamos tomando una muestra aleatoria y este $\hat\beta_T$ no es más que una estimación, es decir, una realización. Esto tiene que ver con que si bien el parámetro $\beta_T$ vale $5$ sus estimaciones no son siempre exactamente $5$. Lo que si nos asegura el experimento aleatorizado es que el estimador $\hat\beta_T$ es un estimador insesgado del parámetro $\beta_T$. Es decir, si repitiéramos el experimento muchas veces, la media de todas las estimaciones $\hat\beta_T$ sería igual a $5$.
...
### La potencia y la correlación intraclase (ICC)
