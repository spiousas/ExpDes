...
Hola y bienvenidos a este repaso de probabilidad y estadística. En esta sección vamos a repasar algunos conceptos básicos de probabilidad y estadística que nos van a ser de vital importancia para lo que sigue. No vamos a entrar en detalle[^recomendacion], ni vamos a hacer demostraciones, sino que vamos a tratar de **entender los conceptos de forma intuitiva y práctica**.

[^recomendacion]: Repetimos la recomendación. Si quieren profundizar en alguno de los temas pueden leer el capítulo 1 de [@wasserman2004all] o el repaso de probabilidad de [@cunningham2021causal] (disponible online).
...
[^intro_stat-1]: Cara será **H**, del inglés *Head* y ceca será **T**, del inglés *Tail*.
...
El concepto de probabilidad es algo complejo, pero, como esto no es un curso de probabilidad, vamos a confiar en que ustedes ya lo tienen claro. Si tienen coraje pueden ir a leer el capítulo 1 de [@wasserman2004all] y, si quieren algo más terrenal, pueden ir a ver el repaso de probabilidad de [@cunningham2021causal] (disponible online). Pero, de forma general, podemos decir que la probabilidad es una medida numérica de la incertidumbre -o la certeza- de que un evento aleatorio ocurra[^freq_vs_bayes].

[^freq_vs_bayes]: Existen dos grandes interpretaciones de la probabilidad: la frecuentista y la bayesiana. La primera define a la probabilidad como la frecuencia relativa de un evento en una serie de experimentos repetidos, mientras que la segunda la define como una medida subjetiva de la creencia o confianza en que un evento ocurra. Si quieren bucear en las profundas aguas de este debate pueden empezar por [acá](https://www.youtube.com/watch?v=GEFxFVESQXc).
...
`r emo::ji("tophat")`¡Matemagia!`r emo::ji("tophat")`
...
Y como las intersecciones son iguales ($P(A_1 \cap B) = P(B \cap A_1)$, podemos despejar de la @eq-proba_total_3 a $P(A_1 B) = P(B|A_1) P(A_1)$ y reemplazarlo en la fórmula de probabilidad condicional:

$$
P(A_1|B) = \frac{P(A_1 \cap B)}{P(B)} = \frac{P(B|A_1) P(A_1)}{P(B)}
$${#eq-proba_total_4}

Pero si recordamos la fórmula de probabilidad total (@eq-proba_total_1), se puede reemplazar como:
...
donde $P(T^+|\neg IKV) = 1-0.9999$ y $P(\neg IKV) = 1-0.0001$. Al reemplazar todos los valores tenemos que:
...
En este caso es muy claro la naturaleza de promedio pesado, ya que a cada valor posible de $X$ lo estamos pesando por su probabilidad. Sin embargo, para una variable aleatoria continua, en la que no tenemos definida una probabilidad puntual $p(x_i)$ sino una función de densidad $f(x)$[^func_densidad], la definición es la siguiente:

[^func_densidad]: La función de densidad $f(x)$ es una función que describe la distribución de probabilidad de una variable aleatoria continua. La probabilidad de que la variable tome un valor dentro de un intervalo se calcula integrando la función de densidad sobre ese intervalo.
...
La varianza nos da una idea acerca de la variabilidad de los procesos aleatorios que generan una variable aleatorio[^varianza]. La varianza de la variable aleatoria $X$ se define como:
...
[^correl]: Si quieren divertirse, y de paso convertirse en ases de la determinación del coeficiente de correlación a *ojímetro*, les recomiendo [este juegazo](https://www.guessthecorrelation.com/). Una estudiante ostenta el abultado récord de $231$ puntos ¿La pasaste?
...
[^abtest]: Probablemente lo más correcto para responder esta pregunta sea un *A/B test*, pero ya hablaremos de eso más adelante.
...
Este nos dice que distribución de la media estandarizada converge endistribución a una $\mathcal{N}(0,1)$ sin importar la distribución de $X$. Esto significa que si el $n$ es lo suficientemente grande podemos estar tranquilos de que no es una asunción tan loca[^TCL].
...
Mientras el $n$ sea más grande, los valores de potencia serán más altos. Es decir, si tengo una muestra más grande voy a cometer menos errores[^fina]. Esta última dependencia es muy importante.
...
El uso que se le da normalmente a esta herramienta es para determinar el mínimo tamaño de muestra necesario para un experimento[^a_priori]. El procedimiento es el siguiente:

[^a_priori]: Algo que se conoce normalmente como *a-priori power analysis*.
...
1. ¿Qué es una variable aleatoria? ¿Cuál es la diferencia entre una variable aleatoria discreta y una continua?

2. ¿Qué es la esperanza de una variable aleatoria? ¿En qué se diferencia de la media muestral? 

3. ¿Por qué decimos que la media muestral es un estimador insesgado de la esperanza?

4. ¿Qué mide la varianza de una variable aleatoria? ¿Y la covarianza entre dos variables? ¿Qué limitación tiene la covarianza como medida de asociación?

5. Tenemos dos pares de variables: **X**, **Y** y **Q**, **P**. Sabemos que $cov(X,Y)= 0.8$, y $cov(Q,P)= 0.059$. Si tuvieras que decir en cuál de los dos pares la relación es más fuerte, ¿qué dirías?

6. ¿Qué es la probabilidad condicional? Escribí su fórmula y explicá con palabras qué significa $P(A|B)$.

7. ¿Qué es el p-valor? 

8. Estás haciendo un test de hipótesis para evaluar la diferencia de medias en dos muestras. Tenés un valor observado que se aleja bastante de lo que esperarías si la hipótesis nula fuera cierta. ¿Qué quiere decir esto?

9. ¿Qué es el error de tipo I y el error de tipo II? ¿Cómo se relacionan entre sí? ¿Qué rol juega α en su control?

10. Estás estudiando datos de accidentes de tránsito en CABA y te das cuenta que un 80% de las personas que presentaron lesiones graves tenían el cinturón de seguridad puesto. Tu amigo llega a la conclusión de que el cinturón de seguridad no sirve para prevenir lesiones graves. ¿Esto es correcto?¿Explicar por qué y cómo le explicarías?

11. ¿Qué es la potencia estadística? ¿De qué factores depende?

12. Estás diseñando un experimento y querés saber a priori qué tamaño de muestra vas a necesitar, ¿que cosas necesitas definir o conocer?¿Por qué?

13. ¿Por qué es importante el Teorema Central del Límite para la inferencia estadística, incluso cuando los datos no siguen una distribución normal?

14. ¿Cuándo se usa la distribución t de Student en lugar de la distribución normal para construir un test? ¿Qué son los grados de libertad?

15. Querés estudiar el nivel de ansiedad en personas de entre 20 y 30 años en Argentina. ¿Cómo pensarías esa población?
...
1. Un funcionario del ministerio afirma: "como el $72%$ se vacunó, el programa fue un éxito y llegó a todos por igual." ¿Qué problema tiene esta afirmación desde el punto de vista estadístico? ¿Qué información adicional necesitarías para evaluarla correctamente?

2. ¿Podés comparar directamente la covarianza entre NSE y cobertura vacunal ($38.4$) con la covarianza entre NSE y asistencia escolar ($310.2)$ para concluir cuál relación es más fuerte? ¿Por qué? ¿Qué harías en cambio?

3. El equipo quiere testear si la media de NSE de las vacunadas es significativamente distinta de la de las no vacunadas. Planteá las hipótesis nula y alternativa, explicá qué significa el p-valor en este contexto y qué error estarían cometiendo si rechazan H₀ cuando en realidad es verdadera.

4. Antes de arrancar el estudio, el equipo quiere determinar el tamaño de muestra necesario. ¿Qué información necesitan para calcularlo? ¿Por qué no alcanza con definir solamente el $\alpha$?

5. El equipo dice que quiere estudiar "la población de adolescentes argentinas". ¿Cómo pensarías esa población en términos del proceso de generación de datos? ¿Es lo mismo que medir a todas las adolescentes del país?
