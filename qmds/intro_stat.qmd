...
Hola y bienvenidos a este repaso de probabilidad y estadística. En esta sección vamos a repasar algunos conceptos básicos de probabilidad y estadística que nos van a ser de vital importancia para lo que sigue. No vamos a entrar en detalle[^profundizar] ni vamos a hacer demostraciones, sino que vamos a tratar de entender los conceptos de forma intuitiva y práctica. 

[^profundizar]: Repetimos la recomendación. Si quieren profundizar en alguno de los temas pueden ir a leer el capítulo 1 de [@wasserman2004all] o el repaso de probabilidad de [@cunningham2021causal] (disponible online).

## Eventos aleatorias

Un evento aleatorio es un resultado posible de un experimento aleatorio. Por ejemplo, si tiramos una moneda al aire, los eventos aleatorios posibles son "cara" y "ceca". Si tiramos dos monedas, los eventos aleatorios posibles son "cara-cara", "cara-ceca", "ceca-cara" y "ceca-ceca". En general, el conjunto de todos los eventos aleatorios posibles se llama espacio de eventos y se lo nombra con la letra $\Omega$.
...
El concepto de probabilidad es algo complejo, pero, como esto no es un curso de probabilidad, vamos a confiar en que ustedes ya lo traen claro. Si tienen coraje puede ir a leer el capítulo 1 de [@wasserman2004all] y si quiere algo más terrenal pueden ir a ver el repaso de probabilidad de [@cunningham2021causal] (disponible online). Pero de forma general podemos decir que la probabilidad es una medida numérica de la incertidumbre o la certeza de que un evento aleatorio ocurra [^freqbay].

[^freqbay]: Existen dos grandes interpretaciones de la probabilidad, la frecuentista y la bayesiana. La primera define a la probabilidad como la frecuencia relativa de un evento en una serie de experimentos repetidos, mientras que la segunda la define como una medida subjetiva de la creencia o confianza en que un evento ocurra. Si quieren bucear en las profundas aguas de este debate pueden empezar por [acá](https://www.youtube.com/watch?v=GEFxFVESQXc).

Como dijimos que tenemos una moneda justa, es razonable pensar que todos los eventos aleatorios en este espacio de eventos $\Omega = \{ TT, TH, HT, HH \}$ son equiprobables, es decir, que cada uno de ellos tiene la misma probabilidad de ocurrir, ¿no? Entonces, la probabilidad de ocurrencia de cada evento será de $1/4$ [^frecuentista]. ¿Pero podemos decir lo mismo de los posibles valores de $X$? ¿Tiene la misma probabilidad de valer $0$ que de valer $1$?

[^frecuentista]: Esto en términos de la intepretación frecuentista significa que si hacemos dos tiradas de monedas infinitas veces, la proporción de veces que saldrá cada una de las combinación es exactamente $1/4$. Y en términos de la interpretación bayesiana... estemmm... bueno... le pueden preguntar a ChatGPT.

Como confío que son gente inteligente (y un poco ansiosa) ya deben haber chusmeado esta tabla y se habrán dado cuenta que no:
...
Este bello y simplísimo ejemplo tiene el único objetivo de pensar un poco para qué sirve una variable aleatoria y cómo es diferente de un evento aleatorios. No vamos a entrar en más detalle.

Cuando las variables aleatorias son continuas la cosa se complica un poco más ya que $P(X=c)$, es decir, la probabilidad de que una variable tome un valor dado, es cero. Esto lo vamos a repensar un poco en la siguiente sección, cuando definamos lo que nos importa para este libro: Las funciónes de densidad y de distribución.

## Probabilidad condicional

La probabilidad condicional es la probabilidad de que ocurra un evento A dado que ocurrió un evento B y se escribe como $P(A|B)$. $P(A|B)$ puede calcularse como:

$$
P(A|B) = \frac{P(A \cap B)}{P(B)}
$${#eq-test}

donde $P(A \cap B)$ es la probabilidad de que $A$ y $B$ ocurran juntos. Esta fórmula se puede entender fácilmente a partir de la definición de probabilidad como frecuencia relativa. Si tenemos una serie de experimentos repetidos, la probabilidad de que ocurra $A$ dado que ocurrió $B$ es simplemente la proporción de veces que ambos eventos ocurrieron juntos ($P(A \cap B)$) sobre la proporción de veces que ocurrió $B$ ($P(B)$). 

Pensemos en el ejemplo de tirar dos monedas justas. Supongamos que queremos saber la probabilidad de que salga exactamente una cara ($A$) dado que salió al menos una cara ($B$). En este caso, $P(A \cap B)$ es la probabilidad de que salga exactamente una cara y al menos una cara, lo cual es simplemente la probabilidad de que salga exactamente una cara ($P(X=1)$), es decir, $P(A \cap B) = P(HT) + P(TH) =1/2$. Por otro lado, $P(B)$ es la probabilidad de que salga al menos una cara ($P(X=1) + P(X=2)$), lo cual es igual a $P(B)1 = P(HT) + P(TH) + P(HH) = 3/4$. Entonces, reemplazando en la fórmula tenemos que:
...
Ahora imaginemos que pasa si queremos calcular la probabilidad de que ocurra B ($P(B)$) a partir de su relación condicional con la ocurrencia de A. Bueno, para esto tendríamos que considerar la probabilidad de que ocurra B dado que ocurrió A y junto con la probabilidad de B dado que NO ocurrió A. A su vez, cada una de estas probabilidades deberíamos pesarlas por la probabilidad de que ocurra o no A. Esto se llama la probabilidad total y se escribe de la siguiente forma[^neg]:
...
`r emo::ji("top_hat")`¡Matemagia!`r emo::ji("top_hat")`

Ya que estamos cancheros, supongamo que tenemos el evento $A_1$ y queremos calcular su probabilidad condicional dado que ocurrió el evento $B$. Recordemos la definción de probabilidad condicional:

$$
P(A_1|B) = \frac{P(A_1 \cap B)}{P(B)}
$${#eq-bay_1}

Pero también $P(B|A_1)$ se puede escribir como:

$$
P(B|A_1) = \frac{P(B \cap A_1)}{P(A_1)}
$${#eq-bay_2}

Y como las intersecciones son iguales ($P(A_1 \cap B) = P(B \cap A_1)$), podemos despejar de la @eq-bay_2 a $P(A_1 \cap B) = P(B|A_1) P(A_1) $ y reemplazarlo en la fórmula de probabilidad condicional:

$$
P(A_1|B) = \frac{P(A_1 \cap B)}{P(B)} = \frac{P(B|A_1) P(A_1)}{P(B)}
$${#eq-bay_3}

Pero si recordamos la fórmula de probabilidad total (@eq-tot_2), $P(B)$ se puede reemplazar como:
...
Ahora que ya llegamos a la fórmula de Bayes a partir de las definiciones de probabilidad total y condicional podemos tomar prestado un ejemplo de [@herzog2019understanding] para entender el valor de las reverendas ideas del Reverendo Thomas Bayes. 

Tenemos un test para identificar si somos portadores de un virus (llamémoslo [IKV](https://www.youtube.com/watch?v=3QH3I1z4Gec)). Este test tiene una sensibilidad de 99.99% y una especificidad de 99.99%. Es decir, la probabilidad de que el test de positivo, dado que tenemos el virus ($P(T^+|IKV)$) es de 0.9999, y lo mismo ocurre para la probabilidad de que el test de negativo en caso de que NO tengamos el virus ($P(T^-|\neg IKV)$). Sabemos también que la incidencia del virus IKV es de 1 en 10000.

Supongamos que somos elegidos aleatoriamente para realizarnos el test y este da positivo ¿Qué probabilidad de ser portadores del virus tenemos ($P(IKV|T^+)$)? La primera respuesta que se nos viene es 0.9999, ¿verdad? Pero, si estuvimos prestando atención, ya a esta altura debemos saber que para invertir la condicionalidad de una probabilidad, de la sensibilidad $P(T^+|IKV)$ a la que nos interesa $P(IKV|T^+)$, tenemos que acudir al bueno de Bayes. O sea:
...
donde $P(T^+|\neg IKV) = 1-0.9999$[^prob_neg] y $P(\neg IKV) = 1-0.0001$. Reemplazando todos los valores tenemos que:

[^prob_neg]: No lo dijimos, pero la probabilidad de que **NO** suceda A ($P(\neg A)$) es simplemente $1-P(A)$.
...
OK, ahora la cosa tiene más sentido. O sea, el test es bastante bueno para decirnos cuando no somos portadores, el problema es cuando da positivo. En este caso tenemos que preocuparnos, pero, como vimos anteriormente, la probabilidad de ser portadores es de apenas 0.5.

Hay una solución más simple para esto y es la que deben estar pensando ustedes: ¿Y si me hacen un segundo test? ¡BINGO`r emo::ji("confetti_ball")`! Calculemos rápidamente la probabilidad de estar infectados si nos testean por segunda vez:
...
La esperanza de una variable aleatoria $X$, a veces también llamada media poblacional, es simplemente la suma pesada de todos sus valores posibles. No debemos confundir la esperanza con el promedio muestral, aunque, como veremos en breve, para algunos casos este último es un estimador insesgado de la esperanza.
...
En este caso es muy claro la naturaleza de promedio pesado, ya que a cada valor posible de $X$ lo estamos pesando por su probabilidad. Sin embargo, para una variable aleatoria continua, en la que no tenemos definida una probabilidad puntual $p(x_i)$ sino una función de densidad $f(x)$[^dens], la definición es la siguiente:

[^dens]: La función de densidad $f(x)$ es una función que describe la distribución de probabilidad de una variable aleatoria continua. La probabilidad de que la variable tome un valor dentro de un intervalo se calcula integrando la función de densidad sobre ese intervalo.
...
La varianza nos da una idea de la variabilidad de los procesos aleatorios que generan una variable aleatoria[^varianza]. La varianza de la variable aleatoria $X$ se define como:
...
Finalmente, tengamos en cuenta que el coeficiente de correlación puede tomar valores entre $-1$ y $1$. Una correlación positiva indica que las variables varían de la misma manera (si aumenta una aumenta la otra) y lo contrario ocurre con una correlación negativa (si aumenta una disminuye la otra). Cuanto más cerca esté el coeficiente de $1$ o $-1$, más fuerte es la relación lineal.

## Población y muestra

Veamos la siguiente figura. En ella podemos ver cómo hay una "población"[^poblacion] de la que queremos saber algo. La teoría de probabilidad nos ayuda a "modelar" esta población, es decir, a definir una función de densidad que nos permita describir la generación de los datos. La inferencia estadística, por otro lado, nos ayuda a estimar los parámetros de esta función de densidad a partir de una muestra de datos. Por supuesto que en estas estimaciones hay aleatoriedad, y si tomamos distintas muestras los resultados van a ser diferentes, pero ahí es donde entra la inferencia estadística, para ayudarnos a medir (o estimar) esta variabilidad y así poder cuantificar la confianza que tenemos en las conclusiones sobre la población a partir de la muestra.

[^poblacion]: Si bien podemos pensar en la población como el conjunto completo de individuos (u objetos o eventos) sobre los cuales se desea obtener información, creo que pensar en el proceso de generación de datos es una forma un poco más completa. Hagamos un pequeño ejercicio mental: si yo les preguntara cuál es el talle de zapatillas promedio de un estudiante de la Universidad de San Andrés. ¿Ustedes creen que la respuesta "poblacional" es medir el pie de todos los estudiantes que están este año en la universidad y calcular el promedio?

![](imgs/probabilidad_e_inferencia.png){fig-align="center"}

Vamos con un ejemplo que nos puede ayudar a entender de qué hablamos cuando hablamos de estimación. Supongamos que conocemos distribución de la altura de la población de varones en Argentina (o sea, conocemos el proceso de generación de datos[^nunca]). No estamos hablando de calcular el promedio de la altura de los varones sino de que conocemos la función de densidad de la cual la altura de cada varón es una muestra. Si no queda del todo claro respiren hondo y esperen un poco que ya se va a ir aclarando. Entonces, la altura de los varones de Argentina tiene una distribución normal[^normal] con media en cm. de $\mu_{varones} = 175$ y una desviación estándar $\sigma_{varones} = 7$, o, como ya aprendimos a decir: $H_{varones} \sim \mathcal{N}(\mu_{varones},\sigma^2_{varones}) = \mathcal{N}(175, 49)$[^intro_stat-2]. A continuación podemos ver la función de densidad:

[^intro_stat-2]: **h** del inglés *height*.
[^nunca]: Está de más decir que NUNCA vamos a conocer esto, o que si lo conociéramos no estaríamos pensando en hacer inferencia ni estimar nada.
[^normal]: Como dijimos anteriormente, este no es un libro de probabilidad, así que si no saben lo que es la distribución normal pueden ir a leerlo a sus apuntes de estadística (procede a preguntarle a ChatGPT "¿Qué es la distribución normal?").
...
Primero definamos nuestra variable aleatoria: $X$:"La diferencia de tiempo en *ms* entre después y antes del cambio". Nuestro objetivo entonces es poder afirmar con cierto grado de seguridad si $E(X)$ es igual a cero o distinto (nos importa tanto si aumenta como si disminuye). Empecemos con lo más sencillo, supongamos que $X \sim \mathcal{N}(\mu, \sigma^2)$. Esta suposición no es tan loca ya que muchos [procesos naturales se distribuyen de forma normal](https://www.youtube.com/watch?v=JX_UyidsQNg&list=PLDcUM9US4XdNOlqSyhe38US8mFgmqzI14&index=10) [^normal_2]. Supongamos también por un momento (más adelante vamos a relajar esta condición) que, ya sea por un experimento previo o porque somos magos, conocemos la $\sigma^2$ de $X$. 

[^normal_2]: Además, como vamos a ver más adelante, cuando el $n$ es lo suficientemente grande, esta condición deja de importar tanto.

Como dijimos en el recuadro, si bien nuestro objetivo es probar si $\mu$ es diferente de $0$, esto lo vamos a hacer a partir de un experimento. Una vez que hagamos el experimento y midamos vamos a tener al clásico estimador de $\mu$: $\hat{\mu}=\bar{X}$, es decir, el promedio muestral. Ahora supongamos que el promedio nos da $0.5$: ¿Es distinto de cero? Es natural pensar que para afirmar eso tenemos que conocer algo de su variabilidad, ¿no? Por ejemplo, no es lo mismo $0.5$ con $20$ mediciones que con $2000$, ¿no? Para responder esa pregunta es que vamos a utilizar las herramientas de la inferencia estadística.

Escribamos las hipótesis que ya mencionamos de forma matemática:
...
Recordemos que si la variable se distribuye así $X \sim \mathcal{N}(\mu, \sigma^2)$ entonces sus medias se distribuyen así[^cuenta] $\bar{X}_n \sim \mathcal{N}(\mu, \sigma^2/n)$[^n], donde $n$ es la cantidad de realizaciones con las que yo calculo mi $\bar{X}$. Entonces si mi $\bar{x}_{obs}$ medida está lo suficientemente lejos de $0$ podemos decir que que $\mu$ es diferente de cero. Pero: ¿Qué es suficientemente lejos?

[^n]: La $n$ en $\bar{X}_n$ es simplemente para enfatizar que es el promedio de una secuencia de $n$ realizaciones.
[^cuenta]: Es una cuenta que aparece en cualquier manual de estadística (procede a preguntarle a ChatGPT).

Bueno, para responder esa pregunta vamos a tener que primero definir los errores que podemos cometer. Por ejemplo, podemos decir que $\mu$ es diferente de cero cuando no lo es (Error tipo I o falso positivo) o podemos decir que $\mu$ no es diferente de cero cuando sí lo es (Error tipo II o falso negativo). El razonamiento tradicional del testeo de hipótesis nulas (NHST[^NHST]) es acotar el error de tipo I.

[^NHST]: Del inglés *Null Hypothesis Significance Testing*.

Para esto vamos a calcular qué tan probable es observar un valor igual o más alejado[^modulo] del cero que $\bar{x}_{obs}$ dado que $H_0$ es verdadera. Es decir, $P(|\bar{X}|>\bar{x}_{obs}|H_0)$. Esta magnitud es lo que se conoce como el viejo y querido p-valor o *p-value* (si te gusta hacerte el canchero). En nuestro caso lo podemos calcular explícitamente. Empecemos estandarizando $\bar{X}$ de la siguiente forma:

[^modulo]: Como pueden observar, el "más alejado" se traduce matemáticamente en un módulo.

$$
Z = \frac{\bar{X} - \mu}{\sqrt{\sigma^2/n}} \sim \mathcal{N}(0,1)
$${#eq-test}

Que bajo $H_0$, como $\mu=0$, se convierte en:

$$
Z_{H_0} = \frac{\bar{X}}{\sqrt{\sigma^2/n}} \sim \mathcal{N}(0,1)
$${#eq-test}

Entonces, si consideramos a $z_{\bar{x}}$ como la versión estandarizada de $\bar{x}_{obs}$[^zbar], podemos calcular el p-valor como:

[^zbar]: $z_{\bar{x}} = \frac{\bar{x}_{obs}}{\sqrt{\sigma^2/n}}$.
...
Y ahora que tenemos esta probabilidad ¿qué hacemos? Bueno, lo que podemos hacer es decir: Si el dato que tomamos viene de un mundo donde $H_0$ es verdadera (también conocido como mundo-$H_0$) podemos calcular que tan "raro" es. Entonces, pongamos una cota en esa probabilidad  de encontrar esos datos y de esa forma estaremos acotando el error de tipo I en el largo plazo[^frecuentista]. Así es que surge el famoso $\alpha$ que normalmente hacemos valer $0.05$[^alpha]. ¿Qué significa eso? Bueno, significa que, si la hipótesis nula fuera verdaera diríamos euivocadamente que hay un efecto cuando no lo hay el $5%$ de las veces. Este párrafo se podría extender al infinito, pero por ahora quedémonos con esta interpretación "práctica" del p-valor (si se quedan con las ganas pueden ir a leer [esto](https://lakens.github.io/statistical_inferences/01-pvalue.html)).
...
Simulemos un experimento para $n=50$ en el que nosotros conocemos tanto $\sigma^2$ como $\mu$[^simulacion]:

[^simulacion]: En este caso vamos a simular un experimento en el que $H_0$ es falsa, es decir, que $\mu$ es diferente de cero y, de hecho, $\mu=0.5$. 
...
La media observada vale `r round(mean(X), 3)` y la media estandarizada ($z_{\bar{x}}$) vale `r round(mean(X)/sqrt(sigma^2/n), 3)`. Miremos como queda $z_{\bar{x}}$ dentro de la distribución de los $Z_{H_0}$ que dijimos arriba (los $\bar{X}$ bajo $H_0$ estandarizados):

```{r echo=FALSE}
#| fig-align: center
#| fig-cap: "Función de densidad de probabilidad de las medias estandarizadas en el mundo en el que la hipótesis nula es verdadera"
#| fig-asp: .5
lims <- tibble(x = qnorm(p = 0.025, mu, sigma), y = c(0.005), 
               xend = qnorm(p = 0.975, mu, sigma), yend = c(0.005))
ggplot() +
  ...
  geom_vline(xintercept = qnorm(p = c(0.025, 0.975), 0, 1), color = "darkorange4", linewidth = .7) +
...
Y el estimador $S^2$ se distribuye tiene una distribución $\chi^2$ con $n-1$ grados de libertad[^chi]:

[^chi]: Para entender esto hay que recordar que $S^2$ es una función de las $n$ realizaciones de $X$ y que la distribución $\chi^2$ se define como la suma de los cuadrados de $n$ variables aleatorias independientes distribuidas normalmente. Entonces, como $S^2$ es una función de las $n$ realizaciones de $X$, y cada una de estas realizaciones se distribuye normalmente, entonces $S^2$ se distribuye como una $\chi^2$ con $n-1$ grados de libertad.

$$
(n-1)\frac{S^2}{\sigma^2} \sim \chi^2_{n-1}
$${#eq-chi}

Y la definición de $t_{n-1}$ es:
...
Calculemos el p-valor de nuestro ejemplo, pero ahora como si no conociéramos la $\sigma$ poblacional[^pt]:

[^pt]: Dentro de la función `pt` ahora se agrega el parámetro `df` que representa los grados de libertad.
...
Para cerrar, vale la pena mencionar que los parámetros $\beta$ de un modelo lineal son variables aleatorias y también tienen distribución t (los grados de libertad son más complejos). O sea que todo lo que vimos hasta acá de errores tipo I, tipo II, p-valor, etc. vale también para ellos.

### ¿Qué pasa si $X$ no se distribuye normalmente?

Ahora que ya vimos que estamos haciendo cuando hacemos inferencia sobre la media, nos damos cuenta que más que interesarnos la distribución de los datos $X$ nos interesa la de sus medias $\bar(X)$ (si estiman algún parámetro de interés, claro). Y acá viene al rescate [el teorema central del límite](https://www.youtube.com/watch?v=zeJD6dqJ5lo):

$$
\frac{\bar{X}-\mu}{\sqrt{\sigma^2/n}}  \xrightarrow{\mathcal{D}} \mathcal{N}(0,1)
$${#eq-CTL}

El mismo nos dice que distribución de la media estandarizada converge en distribución a una $\mathcal{N}(0,1)$ sin importar la distribución de $X$. Esto significa que si el $n$ es lo suficientemente grande podemos estar tranquilos de que no es una asunción tan loca[^TCL].
...
Supongamos que hay una variable aleatoria con distribución exponencial $V \sim \mathcal{E}(\lambda=1)$ tal que $E(V) = \lambda$. Si tomamos una muestra de $n=100$ de $V$, el histograma de la misma se ve algo así:
...
Donde en azul vemos el histograma y en negro la función de densidad para $\mathcal{E}(\lambda=1)$. Ahora, qué pasa si tomamos $1000$ muestras y hacemos el histograma de sus $1000$ medias:
...
Como es de esperarse, la potencia también depende del $\alpha$ ya que cuanto menos estrictos nos ponemos para decir que hay un efecto menos probabilidades de cometer un error de tipo II tenemos (pero más de tipo I pues `r emo::ji("keycap_hash")`**MantaCorta**).
...
[^fina]: Las distribuciones de $H_0$ y $H_1$ se hacen más finas ya que la varianza de la distribución de las medias se divide por $n$ y hay menos solapamiento. 
...
El uso que se le da normalmente a esta herramienta es para determinar el mínimo tamaño de muestra necesario para un experimento[^power]. El procedimiento es el siguiente:

[^power]: Algo que se conoce normalmente como *a-priori power analysis*.
...
3. Una vez que tenemos estas dos magnitudes calculamos el tamaño de muestra para una potencia dada (por ejemplo $0.9$) y un $\alpha$ dado (por ejemplo $0.05$) [^potencia_cuenta].

[^sesoi]: Del inglés *smallest effect size of interest*.
[^potencia_cuenta]: Por supuesto que acá hay que hacer una cuenta, pero excede el alcance de este libro.
...
Para más detalles sobre los procedimientos para justificar el tamaño de muestra ver [@lakens2022sample] o su [versión en OSF](https://osf.io/preprints/psyarxiv/9d3yf_v1) que está actualizada con algunas cositas nuevas.
