...
[^detalles]: Para detalles del ajuste pueden ver el código que acompaña a la Figura XXX.
...
Como vemos en al Figura XXX, depende cómo ajustemos el modelo, el efecto del programa de tutorías cambia. En este caso, el ajuste lineal nos da un efecto de `r round(predict(modelo_cub_1, newdata = tibble(entrance_centered_3 = 0)) - predict(modelo_cub_2, newdata = tibble(entrance_centered_3 = 0)),1)`, el cuadrático de `r round(predict(modelo_sq_1, newdata = tibble(entrance_centered_2 = 0)) - predict(modelo_sq_2, newdata = tibble(entrance_centered_2 = 0)),1)` y el cúbico de `r round(predict(modelo_cub_1, newdata = tibble(entrance_centered_3 = 0)) - predict(modelo_cub_2, newdata = tibble(entrance_centered_3 = 0)),1)`. Es decir, la elección de cómo ajustamos a los datos a cada lado del umbral afecta mi **LATE**.
